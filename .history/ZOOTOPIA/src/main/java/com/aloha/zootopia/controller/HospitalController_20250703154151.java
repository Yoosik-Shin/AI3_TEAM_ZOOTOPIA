...
public class HospitalController {
    ...
    public String list(@RequestParam(value = "animal", required = false) List<Integer> animalIds, Model model) {
        List<Hospital> hospitals = hospitalService.getHospitals(animalIds); // null 이면 전체 조회
        model.addAttribute("hospitals", hospitals);
        model.addAttribute("animalList", hospitalService.getAllAnimals());
        model.addAttribute("selectedAnimals", animalIds); // 체크 유지용
        return "service/hospital/list";
    }
