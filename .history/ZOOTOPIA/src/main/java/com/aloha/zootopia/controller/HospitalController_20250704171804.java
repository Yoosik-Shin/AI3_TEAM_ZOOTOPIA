...
public class HospitalController {
    ...
    public String addReview(@PathVariable Integer id, @ModelAttribute HospReviewForm form, Principal principal, Model model) {
        if (principal == null) {
            // 로그인하지 않은 사용자 처리 (예: 로그인 페이지로 리다이렉트)
            return "redirect:/login"; // 또는 다른 로그인 페이지 경로
        }
        String nickname = principal.getName(); // 사용자 ID를 닉네임으로 사용 (임시)
        // TODO: 실제 userId를 principal.getName()을 기반으로 조회하는 로직 추가
        Integer userId = 1; // 임시로 1로 설정, 실제 사용자 ID로 변경 필요

        model.addAttribute("reviews", reviews == null ? new ArrayList<>() : reviews);
